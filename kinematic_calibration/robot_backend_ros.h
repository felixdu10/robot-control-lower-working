#ifndef ROBOT_BACKEND_ROS_H
#define ROBOT_BACKEND_ROS_H
...
public:
  ...
  void move_raw(std::array<float, 4> pos, float speed = 1.0f) {
    RCLCPP_INFO(
      node_->get_logger(),
...
private:
  ...
  void init_ros() {
    ...
    node_ = rclcpp::Node::make_shared("robot_backend_ros");
  ...
  void send_goal_and_wait(const ActuateSlides::Goal &goal) {
    ...
    send_options.feedback_callback =
        ...
        const std::shared_ptr<const ActuateSlides::Feedback> fb)
      {
        RCLCPP_INFO(
          node_->get_logger(),
          "Feedback: state=%s progress=%.2f",
          fb->state.c_str(),
          fb->progress);
      };

    auto goal_future = action_client_->async_send_goal(goal, send_options);
